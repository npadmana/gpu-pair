...
void CPU_PairCounts::naiveR(const Particles& p1, const Particles& p2, RHist& hh) {
  ...
  for (auto ii = p1.begin(); ii != p1.end(); ++ii) {
    unpack4<float, float, float, int>(*ii, x1, y1, z1, w1);

    // Loop over 2
    for (auto jj = p2.begin(); jj != p2.end(); ++jj) {
      unpack4<float, float, float, int>(*jj, x2, y2, z2, w2);
